...
#SECTION[epic=modification] - start=0, end=0, strobe_gap=0, strobe_on_duration=1, trigger_length=1 are added for selecting the frames of the trigger
    def __init__(self, dataset, trigger_label=0, mode='train', epsilon=0.1, pos='top-left', attack_type='static', time_step=16,
                 trigger_size=0.1, dataname='mnist', polarity=0, n_masks=2, least=False, most_polarity=False, start=0, end=0, strobe_gap=0, strobe_on_duration=1, trigger_length=0, random = False):
        data = None
        ...
        self.start = start
        self.end = end
        self.strobe_gap = strobe_gap
        self.strobe_on_duration = strobe_on_duration
        self.trigger_length = data.shape[1] if trigger_length == 0 else trigger_length
        self.random = random
    ...
    def add_trigger(self, trigger_label, epsilon, mode, type, trigger_size):
        ...
        #ensure that the selected permutation does not contain the target label
        # for p in range(len(perm)):
        #     while new_targets[perm[p]]==trigger_label:
        #         perm[p] = np.random.randint(0, len(new_targets))
...
################################################################################       
        ...
        if len(perm) != 0:
            ...
            # for p in perm:
            #     print(i)
            #     np.savez(f'./to_compare/base/b_{i}.npz', new_data[p])
            #     i+=1
            if type == 'static':
                ...
                new_data[perm] = self.create_flash_trigger(
                    new_data[perm])
            elif type == 'random_flash_adapt':
                new_data[perm] = self.create_random_flash_adapt(
                    new_data[perm])
            else:
            ...
            # for p in perm:
            #     print(i)
            #     np.savez(f'./to_compare/trigger/t_{i}.npz', new_data[p])
            #     i+=1
    ...
    def create_static_trigger(self, data, size_width, size_height, width, height):
        ...
        #SECTION - Adapted static trigger for frames
        # The shape of the data is (N, T, C, H, W)

        frames = get_attack_frames(self,start = self.start, end = self.end, strobe_gap = self.strobe_gap, strobe_on_duration = self.strobe_on_duration, trigger_length = self.trigger_length, random = self.random)
        if polarity == 0:
            data[:, frames, :, y_begin:y_end, x_begin:x_end] = 0
        elif polarity == 1:
            data[:, frames, 0, y_begin:y_end, x_begin:x_end] = 0
            data[:, frames, 1, y_begin:y_end, x_begin:x_end] = 1
        elif polarity == 2:
            data[:, frames, 0, y_begin:y_end, x_begin:x_end] = 1
            data[:, frames, 1, y_begin:y_end, x_begin:x_end] = 0
        else:
            data[:, frames, :, y_begin:y_end, x_begin:x_end] = 1
...
#SECTION - Flash trigger 
    def create_flash_trigger(self,data):
        polarity = self.polarity
        frames = get_attack_frames(self,start = self.start, end = self.end, strobe_gap = self.strobe_gap, strobe_on_duration = self.strobe_on_duration, trigger_length = self.trigger_length, random = self.random)
        if polarity == 0:
            data[:, frames, :, :, :] = 0
...
#!SECTION
    ...
    def create_random_flash_adapt(self,data):
        polarity = self.polarity
        #divide the samples into 4 parts, and each part has a different polarity
        samples0 = data.shape[0]//4
        samples1 = data.shape[0]//4*2
        samples2 = data.shape[0]//4*3

        
        if polarity == 0:
            data[:samples0, :, :, :, :] = 0
        elif polarity == 1:
            data[samples0:samples1, :, 0, :, :] = 0
            data[samples0:samples1, :, 1, :, :] = 1
        elif polarity == 2:
            data[samples1:samples2, :, 0, :, :] = 1
            data[samples1:samples2, :, 1, :, :] = 0
        else:
            data[samples2:, :, :, :, :] = 1
        return data
# def create_defense_data_loader(args):
     # Get the dataset
    # train_data, test_data = get_dataset(args.dataset, args.T, args.data_dir, args.physical)
    ...
    # train_data_ori, test_data_ori = torch.utils.data.random_split(test_data_ori, [int(len(test_data_ori)*0.6), len(test_data_ori)-int(len(test_data_ori)*0.6)])
    
    # clean_train_data_loader = DataLoader(
    #     dataset=train_data_ori, batch_size=args.batch_size, shuffle=True, num_workers=10)
    # test_data_ori_loader = DataLoader(
    #     dataset=test_data_ori, batch_size=args.batch_size, shuffle=False, num_workers=10)
    # test_data_tri_loader = DataLoader(
    #     dataset=test_data_tri, batch_size=args.batch_size, shuffle=False, num_workers=10)

    # return clean_train_data_loader, test_data_ori_loader, test_data_tri_loader

def create_defense_data_loader_physical(args):
     # Get the dataset
    train_data, test_data = get_dataset(args.dataset, args.T, args.data_dir, args.physical)


    train_data_ori = PoisonedDataset(train_data, args.trigger_label, mode='train', epsilon=0,
                                 pos=args.pos, attack_type=args.type, time_step=args.T,
                                 trigger_size=args.trigger_size, dataname=args.dataset,
                                 polarity=args.polarity, n_masks=args.n_masks, least=args.least, most_polarity=args.most_polarity,
                                 start=args.start, end=args.end, strobe_gap=args.strobe_gap, strobe_on_duration=args.strobe_on_duration, trigger_length=args.trigger_length)
    ...
    # frame, _ = test_data_tri[0]
    # play_frame(frame, 'backdoor.gif')

    # print('ehhhhhhhhhhhhhhhhhhhhhhhhhhhhhh')
    clean_train_data_loader = DataLoader(
        dataset=train_data_ori, batch_size=args.batch_size, shuffle=True, num_workers=10)
    test_data_ori_loader = DataLoader(
        dataset=test_data_ori, batch_size=args.batch_size, shuffle=False, num_workers=10)
...
def create_backdoor_data_loader_different(args):
    ...
    test_data_tri_2 = PoisonedDataset(test_data, args.trigger_label, mode='test', epsilon=1,
                                    pos=args.pos, attack_type=args.type, time_step=args.T,
                                    trigger_size=args.trigger_size_2, dataname=args.dataset,
                                    polarity=args.polarity, n_masks=args.n_masks, least=args.least, most_polarity=args.most_polarity,
                                    start=args.start, end=args.end, strobe_gap=args.strobe_gap, strobe_on_duration=args.strobe_on_duration, trigger_length=args.trigger_length)

    # print(args.start)
    # i=0
    # for frame, label in test_data_tri:
    #     if label.argmax().item()==i:
    #         play_frame(frame, f'a{i}.gif')
    #         i+=1

    train_data_loader = DataLoader(
        dataset=train_data, batch_size=args.batch_size, shuffle=True, num_workers=10)
    test_data_ori_loader = DataLoader(
        dataset=test_data_ori, batch_size=args.batch_size, shuffle=False, num_workers=10)
    test_data_tri_loader = DataLoader(
        dataset=test_data_tri, batch_size=args.batch_size, shuffle=False, num_workers=10)
    test_data_tri_loader_2 = DataLoader(
        dataset=test_data_tri_2, batch_size=args.batch_size, shuffle=False, num_workers=10)

    return train_data_loader, test_data_ori_loader, test_data_tri_loader, test_data_tri_loader_2
...
def create_backdoor_data_loader_physical(args):

    # Get the dataset
    train_data, test_data, test_physical_clean, test_physical_poison = get_dataset(args.dataset, args.T, args.data_dir, args.physical)
    ...
    test_data_clean = PoisonedDataset(test_physical_clean, args.trigger_label, mode='test', epsilon=0,
                                    pos=args.pos, attack_type=args.type, time_step=args.T,
                                    trigger_size=args.trigger_size, dataname=args.dataset,
                                    polarity=args.polarity, n_masks=args.n_masks, least=args.least, most_polarity=args.most_polarity,
                                    start=args.start, end=args.end, strobe_gap=args.strobe_gap, strobe_on_duration=args.strobe_on_duration, trigger_length=args.trigger_length)
    test_data_poison = PoisonedDataset(test_physical_poison, args.trigger_label, mode='test', epsilon=0,
                                    pos=args.pos, attack_type=args.type, time_step=args.T,
                                    trigger_size=args.trigger_size, dataname=args.dataset,
                                    polarity=args.polarity, n_masks=args.n_masks, least=args.least, most_polarity=args.most_polarity,
                                    start=args.start, end=args.end, strobe_gap=args.strobe_gap, strobe_on_duration=args.strobe_on_duration, trigger_length=args.trigger_length) 
    #change all the labels of test_data_poison to trigger_label
    ...
    test_data_tri_loader = DataLoader(
        dataset=test_data_tri, batch_size=args.batch_size, shuffle=False, num_workers=10)
    test_data_clean_loader = DataLoader(
        dataset=test_data_clean, batch_size=args.batch_size, shuffle=False, num_workers=10)
    test_data_poison_loader = DataLoader(
        dataset=test_data_poison, batch_size=args.batch_size, shuffle=False, num_workers=10)
    

    return train_data_loader, test_data_ori_loader, test_data_tri_loader, test_data_clean_loader, test_data_poison_loader

def create_expert_data_loader(args):

    # Get the dataset
    train_data, test_data = get_dataset(args.dataset, args.T, args.data_dir, args.physical)

    test_data_clean = PoisonedDataset(train_data, args.trigger_label, mode='train', epsilon=0,
                                    pos=args.pos, attack_type=args.type, time_step=args.T,
                                    trigger_size=args.trigger_size, dataname=args.dataset,
                                    polarity=args.polarity, n_masks=args.n_masks, least=args.least, most_polarity=args.most_polarity,
                                    start=args.start, end=args.end, strobe_gap=args.strobe_gap, strobe_on_duration=args.strobe_on_duration, trigger_length=args.trigger_length)
    test_data_poison = PoisonedDataset(train_data, args.trigger_label, mode='train', epsilon=1,
                                    pos=args.pos, attack_type=args.type, time_step=args.T,
                                    trigger_size=args.trigger_size, dataname=args.dataset,
                                    polarity=args.polarity, n_masks=args.n_masks, least=args.least, most_polarity=args.most_polarity,
                                    start=args.start, end=args.end, strobe_gap=args.strobe_gap, strobe_on_duration=args.strobe_on_duration, trigger_length=args.trigger_length)
    train_data_ori = PoisonedDataset(test_data, args.trigger_label, mode='test', epsilon=0,
                                    pos=args.pos, attack_type=args.type, time_step=args.T,
                                    trigger_size=args.trigger_size, dataname=args.dataset,
                                    polarity=args.polarity, n_masks=args.n_masks, least=args.least, most_polarity=args.most_polarity,
                                    start=args.start, end=args.end, strobe_gap=args.strobe_gap, strobe_on_duration=args.strobe_on_duration, trigger_length=args.trigger_length)
...
def create_backdoor_data_loader_adaptive(args):
    ...
    clean_trainloader = DataLoader(
        dataset=clean_trainset, batch_size=args.batch_size, shuffle=True, num_workers=10)
    bk_trainloader = DataLoader(
        dataset=bk_trainset, batch_size=args.batch_size, shuffle=True, num_workers=10)
    test_data_ori_loader = DataLoader(
        dataset=test_data_ori, batch_size=args.batch_size, shuffle=False, num_workers=10)
    test_data_tri_loader = DataLoader(
        dataset=test_data_tri, batch_size=args.batch_size, shuffle=False, num_workers=10)
...
#SECTION - Get the frames of the trigger
'''
If the end is 0 then it will take all the frames
if the trigger is shorter than the selected frames, it will start at a random point
'''
def get_attack_frames(self, start=0, end=0, strobe_gap=0, strobe_on_duration=1, trigger_length=999, random = False):
    if end == 0:
        end = self.data.shape[1]

    if trigger_length >= end-start:
        trigger_length = end-start
        
    if random:
        #select a random point to start
        start_triger = np.random.randint(start, end-trigger_length)
    else:
        start_triger = start
    ...
    print("a",start_triger)
    print("b",trigger_length)

    #get all the consecutive frames of the trigger leaving a gap of strobe_gap
    for i in range(start_triger,start_triger+trigger_length,strobe_gap+strobe_on_duration):
        if i < end:
            attack_frames.append(i)
        if strobe_on_duration > 1:
            for j in range(1,strobe_on_duration):
                if i+j < end:
                    attack_frames.append(i+j)
    print(str(attack_frames))
