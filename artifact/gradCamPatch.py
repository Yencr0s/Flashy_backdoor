...
def main():
    ...
    ad, clean_testloader, poison_testloader = create_backdoor_data_loader(
            args)
    ...
    for frame, label in poison_testloader:
        #save each frame with the function play_frame and the name of the gif based on i
        #get the index of the 1 in the label tensor 
        label2 = torch.argmax(label)
        if  label2!= 0:
            frame = frame
    ...
    masked_image = apply_mask(input_image, cam, threshold)

    # Save the CAM image for a specific frame

    for frame_idx in range(16):
        combined_image = save_cam_image(cam, masked_image.cpu().detach().numpy(), frame_idx, f'frame_{frame_idx}.png')
...
def apply_mask(input_image, cam2, threshold=0.5):
    #make a tensor of size 16,1,2,128,128
    mask = torch.zeros(16, 1, 2, 128, 128)
    for frame_idx in range(16):
        ...
        # Normalize cam to [0, 1]
        cam = (cam - np.min(cam)) / (np.max(cam) - np.min(cam))
        #make the mask tensor with the values greater than the threshold
        cam = cam > threshold
        mask[frame_idx, 0, :, :, :] = torch.tensor(cam[0, 0, :, :, :], dtype=input_image.dtype)

    mask = mask.to(input_image.device)
    masked_image = input_image * (1 - mask)
    #print('masked',masked_image.shape)
    #print(masked_image)
    # play_frame(input_image[:,0,:,:,:], 'aTRIGGER.gif')
    # play_frame(masked_image[:,0,:,:,:], 'aCLEAN.gif')
    # play_frame(mask[:,0,:,:,:], 'aMASK.gif')
    mask.to('cpu')
    #change the mask to a boolean tensor
    mask = mask.bool()
    del cam
